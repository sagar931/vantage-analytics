...
import { X, Save, BarChart2, TrendingUp, PieChart } from 'lucide-react';

const ChartBuilder = ({ isOpen, onClose, columns, onSave }) => {
  const [title, setTitle] = useState('');
  const [type, setType] = useState('bar'); // bar, line, area
  ...
  const handleSave = () => {
    ...
    const newChart = {
      ...
      xAxis,
      dataKeys,
      colors: ['#3b82f6', '#10b981', '#f59e0b', '#ef4444'] // Default Theme Colors
    };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <input 
                ...
                placeholder="e.g. PSI Trend vs Volume"
              />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-2">X-Axis (Category)</label>
            ...
            {/* Y-Axis (Multi Select) */}
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Y-Axis (Values)</label>
              <div className="bg-slate-800 border border-slate-700 rounded-lg p-2 max-h-48 overflow-y-auto custom-scrollbar">
                {columns.map(col => (
                  <button
                    key={col}
